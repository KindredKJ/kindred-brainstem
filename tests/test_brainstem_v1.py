...
from brainstem.engines.kindred_revenue_stack_engine.engine import (
    KindredRevenueStackEngine,
)
...
def test_health_and_krse_import():
    ...
    assert r.exit_code == 0 and (
        "OFFLINE" in r.output or "DEGRADED" in r.output or "HEALTHY" in r.output
    )
    assert KindredRevenueStackEngine().generate_plan({"name": "x"}).product_name == "x"


def test_reality_loop_and_external_level_gate():
    _c, p = create_contract("products/moneyback_scan.yaml")
    assert p.exists()
    e = execute("products/moneyback_scan.yaml")
    assert Path(e["artifact"]).exists()
    v = verify("products/moneyback_scan.yaml")
    assert v["level"] == 4
...
def test_claim_guard_approval_plugcore_model_memory_backprop_attention():
    cg = review("moneyback_scan", ["revenue_generating", "tax_ready"])
    assert (
        cg["results"][0]["safe_claim"] == "revenue_path_defined"
        and cg["results"][1]["safe_claim"] == "tax_professional_review_packet_needed"
    )
    ar = appr.request("public_launch", "moneyback_scan")
    assert ar["status"] == "pending" and appr.blocks("public_launch", ar["approval_id"])
    ...
    mem.add(
        ...
        "result,verification,external proof",
        "Only verified results count.",
    )
    assert mem.recall("external proof")
...
def test_audit_and_transition_and_hash_chain(tmp_path):
    sess = audit.start("global founder asset and revenue audit")
    assert sess["status"] == "evidence_needed"
    acsv = tmp_path / "assets.csv"
    acsv.write_text(
        "asset_id,name,category,owner_reported,related_entity,description,value_estimate_usd,tax_relevance,notes\na1,Brainstem,software_project,Kindred,,desc,0,yes,n\n"
    )
    rcsv = tmp_path / "rev.csv"
    rcsv.write_text(
        "revenue_id,source_name,source_type,related_entity,related_product,processor,amount,currency,transaction_date,tax_year,evidence_ref,notes\nr1,Mock,manual,,,none,0,USD,2026-01-01,2026,MOCK,n\n"
    )
    ...
    tr = trans.transition_record()
    assert tr["legal_completion_claimed"] is False and trans.report().exists()
    path = ROOT / "data" / "evidence_ledger.jsonl"
    r1 = append_jsonl(path, {"x": 1})
    r2 = append_jsonl(path, {"x": 2})
